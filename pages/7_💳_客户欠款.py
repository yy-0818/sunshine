...
def render_analysis_view(df, title, icon):
    ...
    with c2:
        ...
        fig_pie.update_traces(
            hovertemplate='<b>客户类型:</b> %{label}<br>' +
            '<b>客户数量:</b> %{value}人<br>' +
            '<extra></extra>'
        )
...
def render_comprehensive_tab(debt_service):
    ...
    with st.container(border=True):
        ...
        all_search = col_s1.text_input("全局搜索", placeholder="输入客户名称或客户编号...", key="all_search")
    ...
    if all_search:
        # 确保客户代码列是字符串类型以便搜索
        df_view['客户代码_str'] = df_view['客户代码'].astype(str)

        # 同时搜索客户名称和客户代码
        name_mask = df_view['客户名称'].str.contains(all_search, case=False, na=False)
        code_mask = df_view['客户代码_str'].str.contains(all_search, case=False, na=False)
        
        # 使用逻辑或组合两个条件
        df_view = df_view[name_mask | code_mask]
        
        # 删除临时列
        df_view = df_view.drop(columns=['客户代码_str'])
    if dept_filter:
    ...
    styled_view = apply_style(df_view[['来源部门', '客户代码', '客户名称', '2025欠款', '23-25总变化', '坏账风险', '客户价值等级']])
...
def render_classification_help_tab(debt_service):
    ...
    try:
        explanation_data = debt_service.get_classification_explanation()
    except Exception as e:
        st.error(f"获取分类说明失败: {str(e)}")
        explanation_data = {
            '客户类型': {
                '优质客户(无欠款)': '当前无任何欠款',
                '新增欠款': '去年无欠款，今年新增',
    ...
    advice_data = [
        {"客户等级": "A级/B级", "风险状态": "无风险/正常跟踪", "管理策略": "正常维护", "具体措施": "定期对账，保持良好关系", "建议频率": "季度跟进",},
        {"客户等级": "C级", "风险状态": "关注类", "管理策略": "重点关注", "具体措施": "了解欠款增加原因，确认还款计划", "建议频率": "月度跟进",},
        {"客户等级": "D级", "风险状态": "中风险坏账", "管理策略": "强力催收", "具体措施": "停止赊销，发催款函，专人跟进", "建议频率": "每周跟进",},
        {"客户等级": "E级", "风险状态": "高风险坏账", "管理策略": "法律介入", "具体措施": "发律师函，准备诉讼，资产保全", "建议频率": "立即跟进",}
    ]
