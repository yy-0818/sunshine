...
if not table_names:
    ...
    if selected_table:
        ...
        if not table_data.empty:
            ...
            # 计算分页
            page_size = st.selectbox("每页显示行数", [50, 100, 200, 500], index=0, key="page_size")
            total_pages = max(1, (len(table_data) + page_size - 1) // page_size)
            page_number = st.number_input("页码", min_value=1, max_value=total_pages, value=1, key="page_number")
            ...
            with col1:
                st.selectbox("每页显示行数", [50, 100, 200, 500], index=0, key="page_size_bottom")
            with col2:
                st.write("")  # 占位
            with col3:
                total_pages_bottom = max(1, (len(table_data) + page_size - 1) // page_size)
                st.number_input("页码", min_value=1, max_value=total_pages_bottom, value=page_number, key="page_number_bottom")

            # 分页信息
            st.caption(f"显示第 {start_idx + 1} - {min(end_idx, len(table_data))} 行，共 {len(table_data)} 行")
